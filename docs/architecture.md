...
**Warm-up mail bypasses this path entirely.** Before classification, `processInbound` checks for a verified `X-Inroad-Warmup` token; a warm-up message is recorded and returns *before* `processMessage`, so it never reaches reply/bounce handling (see the Warm-up engine below and `docs/security.md` invariant 25).

## Warm-up engine
Warm-up builds and maintains mailbox sender reputation by having a workspace's own opted-in mailboxes exchange low-volume, human-looking mail and engage with it the way a real recipient would. It reuses the existing send seam, SSRF guard, claim-before-send idempotency, `coreapi` boundary, and workspace-pinning — it adds no new worker→DB path. Design spec: `docs/superpowers/specs/2026-07-26-warmup-engine-design.md`; migration `000018_warmup` (+ `000019` receipt locator).

- **Pool model — workspace-local.** A mailbox only warms other enabled, healthy participants in the SAME workspace (`warmup_participants`), so every query stays `workspace_id`-pinned and no mail crosses tenants. A workspace needs ≥2 participants; below that the engine idles (surfaced in the UI).
- **The loop.** (1) **Send** — the `warmup:tick` worker claims and sends a warm-up email from a mailbox to a chosen partner: a new thread, or (weighted by the participant's `reply_rate`) a reply on an existing open thread. Content comes from a curated static library behind an injected `warmup.ContentGenerator` seam (an AI generator drops in later, exactly like the reply-classifier's `ModelClassifier`). Each send carries a signed `X-Inroad-Warmup` receipt token. (2) **Detect** — the recipient's inbox poller scans INBOX *and* the provider's spam folder, verifies the token, records a `warmup_receipt` with the placement (inbox / spam / other), and — critically — **isolates** the message from campaign classification. (3) **Engage** — a delayed `warmup:engage` task rescues the message from spam (IMAP `MOVE`, Gmail label modify), marks it read, and sometimes replies in-thread (a new warm-up send with its own claim + token). (4) **Health** — placement is attributed to the **sender** (`warmup_sends.from_mailbox`); `EvaluateWarmupHealth` rolls a trailing spam-placement rate into a state machine `healthy → watch → throttled → paused` that dampens the next ramp target and, when paused, halts sending until a clean window elapses.
- **Scheduling.** A `warmup:sweep` scheduler tick (`@every 5m`) fans out due mailboxes (ramp target from `warmup.RampTarget` + a deterministic per-day volume factor + jitter, minus the send/spacing already used today) and runs the health evaluation. The send worker also lazily chains the next tick after each send.
- **Seams.** Control plane: `internal/app/warmup/` (store/service/handler for the `PUT/DELETE/GET /mailboxes/{id}/warmup` + `GET /warmup/overview` API) and the `coreapi` warm-up methods (send-job, claim, receipt, engage-job, health). Execution plane: `internal/worker/warmup/{send,engage,sweep}.go` + the receipt hook in `internal/worker/inbox/poll.go`. Pure, offline, testable helpers (content library, HMAC token, ramp/jitter/health math) live in `internal/platform/warmup/`; the recipient-side write actions are behind the `mail.Engager` seam (IMAP + Gmail; M365 is a documented follow-up).

## Worker routing & transport
Two pieces of distributed-worker groundwork landed with warm-up (they are transport-neutral and used by campaign sends too, once wired):

- **Transport-bus seam (`internal/platform/bus`).** A small `Dispatcher`/`PeriodicScheduler` interface expressing delivery *intent* (deliver-once, deliver-at, deliver-after, periodic). The only shipped implementation is `redisbus` over the existing asynq client; a Kafka/NATS implementation drops in without touching domain or worker code — the same "interface + one impl" pattern as `coreapi` and the reply-classifier model seam. (See ADR 0002.)
- **Per-IP worker routing.** A worker heartbeats into a global `workers` registry with its `INROAD_WORKER_ID` + optional `INROAD_WORKER_EGRESS_IP`; `AssignMailboxWorker` least-loaded-assigns each mailbox to a live worker (`mailbox_worker_assignments`, workspace-pinned; migration `000017`) so a mailbox's outbound mail consistently egresses from one worker/IP. The worker's outbound dialer binds that egress IP as the *source* address only — it never bypasses the `vetAddr` destination check (`docs/security.md` invariants 22–24). Single-node dev degrades cleanly to a shared default queue.
