...
| P3.8 | **Physical control/execution split** — worker gets a client not a `pgxpool` for the ~26 per-message `coreapi` methods a `send`-role host actually calls, plus join-token enrollment | L | **Partially done.** Fleet F2 (#207) shipped the internal listener and DEK decryption-behind-a-broker (`internal/platform/credbroker`) already — a `send` host can run with `INROAD_MASTER_KEY` unset today. What remains is narrower: the business-data half of `coreapi.Client` still needs `INROAD_DATABASE_URL`, and there is no join-token enrollment flow. Staged plan: [`05-coreapi-http-transport-plan.md`](05-coreapi-http-transport-plan.md). |
| P3.9 | **Mailbox → worker assignment engine** — assign by health band + load across the per-IP queues that already exist; fleet health quarantine + rebalance | L | **Mostly done**, per the "Undocumented epic" note above — F3/F4 (`#198`, `#206`) built scored placement, health-gated routing, and an append-only decision log without waiting for P3.8's literal HTTP split. What's left is P3.8-shaped, not P3.9-shaped: the assignment logic already exists, it just can't yet run on a host that doesn't hold direct DB credentials. |
