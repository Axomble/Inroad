...
| P2.22 | **Warmup reputation must follow the address, not the row** — `warmup_participants` is keyed `mailbox_id PRIMARY KEY … ON DELETE CASCADE` with a live `DELETE` path, so lane and health state die with the row: disconnect a mailbox and re-add the same address and every accumulated penalty is erased. Mirror the standing to a ledger keyed on `(workspace_id, email)`, maintained by a trigger so no caller can bypass it, and read the **worst** standing on rejoin | S–M | **Added 2026-09-14.** A correctness bug, not a feature gap — it silently hands a penalised address a clean slate, which is precisely the mailbox that should not get one. The reference platform shipped the identical fix after hitting it on paths that never touched the mailbox (auth error, lapsed plan, warmup toggled off). Confirm which paths actually delete before designing. |
