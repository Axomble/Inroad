...
func TestUnsubscribeGETDoesNotMutate(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(context.Background(), http.MethodGet, "/u/"+tok, http.NoBody)
...
func TestUnsubscribePOSTInserts(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(context.Background(), http.MethodPost, "/u/"+tok, http.NoBody)
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
