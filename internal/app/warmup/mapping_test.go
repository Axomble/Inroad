...
func TestPlacementRateFromGen(t *testing.T) {
	mailboxID := uuid.MustParse("55555555-5555-5555-5555-555555555555")

	cases := []struct {
		name string
		in   gen.GetWarmupPlacementRates7dRow
		want PlacementRate
	}{
		{
			name: "zero value maps to zero domain",
		...
		{
			name: "sums project 1:1 including mailbox id",
			in: gen.GetWarmupPlacementRates7dRow{
				MailboxID: mailboxID,
				Inbox:     4,
				Spam:      2,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := placementRateFromGen(tc.in); !reflect.DeepEqual(got, tc.want) {
				t.Errorf("placementRateFromGen mismatch\n got: %+v\nwant: %+v", got, tc.want)
			}
		})
	}
}
