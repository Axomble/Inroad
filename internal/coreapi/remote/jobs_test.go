...
func serveJobs(t *testing.T, jobs *fakeJobs, opener *fakeOpener) (*Client, *httptest.Server) {
	t.Helper()
	h, err := NewHandler(Deps{Suppression: &fakeSuppression{}, Jobs: jobs}, testToken, quietLogger())
	if err != nil {
...
// A handler missing either half refuses to be built. Half a transport would
// start, register its routes, and fail every call to the other half at the
// first send.
func TestAHandlerNeedsBothHalvesWired(t *testing.T) {
	if _, err := NewHandler(Deps{Jobs: &fakeJobs{}}, testToken, quietLogger()); err == nil {
		t.Error("a handler with no suppression reader was built")
	}
	if _, err := NewHandler(Deps{Suppression: &fakeSuppression{}}, testToken, quietLogger()); err == nil {
