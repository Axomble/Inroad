...
func serve(t *testing.T, r SuppressionReader, clientToken string) (*Client, *httptest.Server) {
	t.Helper()
	h, err := NewHandler(Deps{Suppression: r, Jobs: &fakeJobs{}, Outcomes: &fakeOutcomes{}, InboxSends: &fakeInboxSends{}, Inbound: &fakeInbound{}, Fleet: &fakeFleet{}}, testToken, quietLogger())
	if err != nil {
...
func TestAMissingAuthorizationHeaderIsRejected(t *testing.T) {
	f := &fakeSuppression{answer: true}
	h, err := NewHandler(Deps{Suppression: f, Jobs: &fakeJobs{}, Outcomes: &fakeOutcomes{}, InboxSends: &fakeInboxSends{}, Inbound: &fakeInbound{}, Fleet: &fakeFleet{}}, testToken, quietLogger())
	if err != nil {
		t.Fatalf("NewHandler: %v", err)
	}
	srv := httptest.NewServer(h)
...
func TestAWeakTokenIsRefusedOnBothSides(t *testing.T) {
	...
	if _, err := NewHandler(Deps{Suppression: &fakeSuppression{}, Jobs: &fakeJobs{}, Outcomes: &fakeOutcomes{}, InboxSends: &fakeInboxSends{}, Inbound: &fakeInbound{}, Fleet: &fakeFleet{}}, short, quietLogger()); !errors.Is(err, credbroker.ErrWeakToken) {
