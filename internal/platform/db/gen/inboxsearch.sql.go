...
page AS (
    ...
    WHERE t.workspace_id = $4
      ...
      AND ($5::uuid IS NULL OR t.mailbox_id = $5)
      AND ($6::text IS NULL OR t.last_reply_class = $6)
      AND ($7::timestamptz IS NULL
           OR (t.last_message_at, t.id) < ($7::timestamptz, $8::uuid))
      AND (NOT $9::boolean OR t.unread)
      AND ($10::timestamptz IS NULL
           OR t.last_message_at >= $10::timestamptz)
      AND (NOT $11::boolean
           OR inbox_thread_awaiting_reply(t.id, t.workspace_id, t.campaign_id, t.contact_id))
      AND (NOT $12::boolean OR NOT EXISTS (
...
LEFT JOIN LATERAL (
    ...
    FROM (
        ...
        WHERE p.matched_outbound
          ...
          AND s.sent_at IS NOT NULL
          AND inbox_search_document(COALESCE(v.subject, st.subject), COALESCE(v.body_text, st.body_text)) @@ inbox_search_query($3::text)
    ) candidates
