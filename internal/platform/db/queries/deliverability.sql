...
sent AS (
    ...
    WHERE s.workspace_id = @workspace_id AND s.status = 'sent'
      AND s.sent_at >= ((@since::date)::timestamp AT TIME ZONE 'UTC')
    GROUP BY 1
),
bounced AS (
    SELECT day, COUNT(*) AS n FROM (
        ...
        WHERE e.workspace_id = @workspace_id AND e.stop_reason = 'bounced'
          AND e.stopped_at >= ((@since::date)::timestamp AT TIME ZONE 'UTC')
        UNION
        ...
        WHERE ev.workspace_id = @workspace_id AND ev.kind = 'bounce'
          AND ev.received_at >= ((@since::date)::timestamp AT TIME ZONE 'UTC')
    ) u GROUP BY day
),
complained AS (
