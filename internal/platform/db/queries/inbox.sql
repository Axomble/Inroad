...
-- name: ListStrandedInboxPendingReplies :many
...
WHERE send_after <= now()
  AND ((status = 'scheduled' AND send_after < now() - @overdue_after::interval)
       OR (status = 'sending' AND claimed_at < now() - @stale_claim_after::interval))
ORDER BY send_after
LIMIT 200;

-- name: ListStrandedInboxPendingComposes :many
