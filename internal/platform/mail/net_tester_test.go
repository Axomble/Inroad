...
func TestSMTPRejectsLoopback(t *testing.T) {
	...
	err := tester.TestSMTP(SMTPConfig{Host: "127.0.0.1", Port: 587})
...
func TestSMTPRejectsCloudMetadataLinkLocal(t *testing.T) {
	...
	err := tester.TestSMTP(SMTPConfig{Host: "169.254.169.254", Port: 587})
	if !errors.Is(err, ErrHostNotPermitted) {
...
func TestSMTPRejectsDisallowedPort(t *testing.T) {
	...
	err := tester.TestSMTP(SMTPConfig{Host: "203.0.113.10", Port: 6379})
	if err == nil {
