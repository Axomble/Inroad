...
func TestInboxIntegrationReplyStopsEnrollment(t *testing.T) {
	...
	if err := PollHandler(fx.core, reader, nil, nil)(ctx, pollTaskFor(t, fx.mailboxID.String(), fx.ws.String())); err != nil {
		t.Fatalf("poll: %v", err)
	}
...
func TestInboxIntegrationRepollIsIdempotent(t *testing.T) {
	...
	if err := PollHandler(fx.core, reader, nil, nil)(ctx, task); err != nil {
