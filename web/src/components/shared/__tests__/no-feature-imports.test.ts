...
// The directory under guard is the parent — this test lives one level down in
// `__tests__/`, and pointing `sourceFiles` at its own folder would scan nothing
// and pass vacuously.
...
test('no shared component depends on a feature', () => {
  const offenders = sourceFiles(sharedDir)
    .map((path) => ({ path, source: readFileSync(path, 'utf8') }))
    .filter(({ source }) => /from '@\/features\//.test(source))
    .map(({ path }) => path.slice(sharedDir.length + 1))
