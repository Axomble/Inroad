...
const HEALTH_REASONS: Record<string, string> = {
  ...
  placement_sample_insufficient: 'Not enough fresh results to judge this mailbox — unproven, not clean',
  insufficient_evidence_to_recover: 'Held where it was: not enough fresh results to recover',
  recovery_step: 'A clean stretch of results, so it recovered one step',
  recovery_blocked_by_dwell: 'Recovery is held back until the current pause has run its minimum time',
  evidence_qualified: 'Enough clean results to establish this mailbox as healthy',
...
const LANE_REASONS: Record<string, string> = {
  ...
  lane_auth_regressed: 'Domain authentication stopped passing, so the mailbox went back to waiting on DNS',
  lane_admitted_to_probation: 'Domain authentication passed, so it started a proving period at a low daily volume',
  lane_quarantined: 'Paused for safety and withheld from warmup — no warmup mail and no new campaign leads until fresh clean results clear it',
  lane_quarantine_held: 'Still withheld: time alone never clears a safety pause — it takes fresh clean results and passing domain authentication',
  lane_quarantine_resumed: 'Domain authentication passed, but the safety-pause cooldown has not finished yet',
  lane_cooldown_active: 'The safety-pause cooldown is still running',
  lane_cooldown_elapsed: 'Cooldown finished, so it moved to recovery to earn its way back at a low daily volume',
  lane_qualified: 'A clean stretch of results, so it was promoted to full participation',
  lane_awaiting_evidence: 'Waiting for a clean stretch of results before it can be promoted',
  lane_recovered: 'Clean results, so it returned to full participation',
  lane_watch: 'Being watched — volume reduced while the signal is checked',
  lane_watch_held: 'Still being watched until clean results arrive',
  lane_evidence_lapsed: 'No fresh clean results, so it went back to proving itself',
  lane_healthy: 'Clean results — participating fully in warmup',
  lane_blocked_held: 'Blocked — someone on your team has to approve re-entry before recovery can start',
}
