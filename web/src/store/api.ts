...
export type WarmupDetail = {
  ...
  /** Placement split by WHERE THE MAIL WAS DELIVERED — the destination ESP, decided by the recipient domain's MX. Each route carries its OWN sample count and its rates are computed over that count, never the mailbox's pooled total. Empty when nothing has been observed. Reported for visibility only: no threshold, lane or promotion decision reads any of it. Warmup partners are the workspace's own connected mailboxes, so the only routes observable are the ESPs in that pool: a single-ESP workspace reports ONE route, and a client must render that as "one destination observed" rather than as a matrix that happens to have one clean row — it says nothing about delivery to any provider absent from the pool. `unknown` is not a fourth provider: it means the recipient domain's MX has not been resolved yet, and must never render beside google/microsoft/other as if it were a destination. `other` is its opposite — resolved, and neither Google nor Microsoft. */
  routes?: {
    /** Where the mail was delivered, from the recipient's own provider or the recipient domain's MX cache — never from the sender's relay. Recorded on the observation at receipt time, so a later provider or MX change does not rewrite which route historical mail was measured on. unknown = not resolved yet (an absence of lookup); other = resolved, and neither Google nor Microsoft (a finding). The two must not render alike. */
    destination_esp: "google" | "microsoft" | "other" | "unknown";
    /** Inbox plus spam placement observations ON THIS ROUTE in the trailing 7 days — the denominator of this row's inbox and spam rates, and never the mailbox's pooled placement_sample_7d. */
    placement_sample_7d: number;
    /** Fraction of this route's placements that reached the inbox (0..1). Null when this route is below the placement sample floor: not established, not zero. Splitting a window by destination shrinks every count, so null is common here rather than exceptional. */
    inbox_rate_7d: number | null;
    /** Fraction of this route's placements that landed in spam (0..1). Null when this route is below the placement sample floor — not established, and never to be rendered as a clean 0%. */
    spam_rate_7d: number | null;
    /** Fraction of this route's tab-capable placements that landed in a tab rather than the primary inbox (0..1), over tab_capable_sample_7d. Null when nothing observing this route could report a category. */
    tabbed_rate_7d: number | null;
    /** Inbox-side placements on this route whose reader could have seen a tab — the denominator of this row's tabbed_rate_7d, and smaller than this row's placement_sample_7d by construction. */
    tab_capable_sample_7d: number;
  }[];
};
...
export type WarmupMailbox = {
  ...
  placement_sample_7d: number;
  /** Of this mailbox's warmup mail that a provider could categorise, the fraction that landed in a TAB (Gmail Promotions/Updates/Social/Forums) rather than the primary inbox. CATEGORY_PERSONAL is not a tab — Gmail applies it to ordinary Primary-tab mail, so counting it would report almost everything as tabbed and invert the signal. A tabbed landing still counts as an inbox placement in placement_sample_7d and inbox_rate_7d: it arrived, just not in Primary, and excluding it would strip a Promotions-only mailbox of every placement sample and demote it to unknown. Null when nothing that OBSERVED this mailbox's mail could report a category. Note whose capability that is: placement is attributed to the SENDER but recorded by the RECIPIENT's poller, so a Gmail sender whose warmup partners are all IMAP reports null even though its own provider could categorise perfectly well. Render it as "no partner could report a tab", never as a clean zero, and never as a statement about this mailbox's own provider. Reported for visibility only: no threshold, lane or promotion decision reads it, because a signal invisible on a whole provider class must not gate anything. */
  tabbed_rate_7d?: number | null;
  /** INBOX-SIDE placement observations in the window whose reader could have seen a tab — the denominator of tabbed_rate_7d. Spam-placed observations are excluded deliberately: a tab is a distinction WITHIN the inbox, so folding spam into this denominator would make a mailbox with a spam problem read as having less of a tab problem. Counted separately from placement_sample_7d because pooling observations that structurally cannot report a tab would dilute the rate toward zero and make an untested pool read clean. */
  tab_capable_sample_7d?: number;
